...
| 状态 | 图标 | 含义 | 工具提示 |
|------|------|------|---------|
| **关闭**（原始文本） | `📝` 或 `markdown` 图标 | 点击后渲染 | "Render Markdown" |
| **开启**（已渲染） | `👁️` 或 `eye` 图标 | 点击后显示源码 | "Show Source" |

### 2. 视觉样式

**悬浮按钮容器**:
- 定位: `absolute top-3 right-3 z-10`
...
// 渲染切换状态 (持久化存储，按文件路径存储)
const storageKey = () => `md-render-state:${path() ?? ""}`
const [renderMarkdown, setRenderMarkdown] = makePersisted(
  createSignal(false),
  { name: "md-render-toggle", storage: localStorage }
)
```
...
- 使用 `makePersisted` 实现状态持久化（来自 `@solid-primitives/storage`）
- 存储键基于文件路径，实现每个文件独立记忆
- 首次打开文件时读取上次的状态
...
const MarkdownToggleButton = () => (
  <Show when={isMarkdown()}>
    <div class="absolute top-3 right-3 z-10 flex items-center gap-1 bg-bg-surface/90 border border-border-base rounded-lg p-2 shadow-md transition-all duration-200">
      <IconButton
...
│           ├── Match: state()?.loaded
│           │   └── renderFile(contents())
│           │       ├── MarkdownToggleButton (条件显示，悬浮)
│           │       │   └── Show: isMarkdown()
│           │       │       └── div.floating-button (absolute top-3 right-3)
│           │       │           └── IconButton (icon: "file-text" | "eye")
│           │       └── Switch (流程控制)
...
| `renderMarkdown` | `Signal<boolean>` | `false` | ✅ (localStorage) | 永不重置（持久化） |

**存储策略**:
- 存储位置: `localStorage`
- 存储键: `md-render-toggle:${filePath}`
- 存储值: `true` 或 `false`
- 作用域: 每个文件独立记忆

**状态流**:
```
文件打开 → 检测扩展名 → isMarkdown = true/false
                      ↓
              如果是 Markdown
                      ↓
...
| 测试场景 | 前置条件 | 操作步骤 | 预期结果 |
|---------|---------|---------|---------|
| **TC-01**: 打开 .md 文件 | 无 | 打开 `README.md` | 显示原始文本，悬浮按钮可见，显示 `file-text` 图标 |
| **TC-02**: 切换到渲染视图 | TC-01 完成 | 点击悬浮按钮 | 内容切换为渲染后的 HTML，图标变为 `eye` |
| **TC-03**: 切换回原始视图 | TC-02 完成 | 再次点击悬浮按钮 | 内容恢复原始 Markdown 文本，图标变为 `file-text` |
| **TC-04**: 状态持久化 | TC-02 完成 | 关闭文件后重新打开 | 保持上次的渲染状态（渲染视图） |
| **TC-05**: 切换不同文件 | TC-02 完成 | 打开另一个 `.md` 文件 | 读取该文件上次保存的状态 |
